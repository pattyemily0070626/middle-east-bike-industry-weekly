...
- 受眾：**台灣自行車產業中小企業（SME）**——零件廠、A-Team 外圍、配件廠、上下游 supplier
- 風格：簡潔、數據密集、聚焦「中東情勢 × 台灣自行車產業」的具體傳導；敘述強調「對 SME 的訊號意義」，而非廠商自身策略細節
- 語言：繁體中文（台灣）

## 語言規則（四項撰寫規則，詳見 README ## 撰寫規則）

1. **出口動能指標不空白**：同月跑第二次週報時，「自行車出口動能」仍依當月初步值給判讀（如「分化（電動自行車續弱）」），不可寫「待 X 月確定值」或留空。
2. **全文繁體中文**：外電摘錄、論點、引述語一律譯為繁中；引述句可標「（中譯）」。專有名詞與品牌名保留原文。
3. **e-bike → 電動自行車**：所有 e-bike / E-bike / ebike 寫作「電動自行車」；品牌名中的 eBike（如 Porsche eBike Performance）可保留。
4. **英文縮寫加註中文**：BAF / PSS / FAK / OEM / ODM / WCI / IACI / SCFI / UCI / HS code 等首次出現時加括號中文全稱與說明，例如「BAF（Bunker Adjustment Factor，燃油附加費）」。同檔後續可省略。
- 表格中的評估等級用中文：「高 / 中 / 低」「重大 / 中等」「上升 / 持平 / 下降」「疲弱 / 持平 / 強 / 分化」。
...
|---|---|
| 中東航運風險 | 高 / 中 / 低 |
| WCI 運價走勢 | 上升 / 持平 / 下降 / 續漲（連 N 週） |
| 自行車出口動能 | 疲弱 / 持平 / 強 / 分化（XX 續弱） |
